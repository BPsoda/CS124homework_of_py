...
def subList(t, b): 
    ''' substracts a small positive list number from a big one '''
    result=[]
    a = deepcopy(t)
...
def str2list(stri): 
    '''turns a string number into a list'''
    lst=[]
    if stri[0] == "-":
        for i in stri[-1: 0: -1]:
            lst.append(int(i))
...
def list2str(lst, sign): 
    '''turns a list number into a string'''
    if sign== False:
